...
from ....core.synthesis.mixed import generate_voxels_from_sample_id
...
def _cmd(config: ProjectConfig, args: argparse.Namespace) -> None:
    ...
    voxels, metadata = generate_voxels_from_sample_id(
        ...
        cmdconfig.mixed.num_cells,
        cmdconfig.mixed.radius_min,
        cmdconfig.mixed.radius_max,
    )
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "synthesize-mixed",
        help="Generate a mixed swiss cheese voxel model.",
    )
    parser.add_argument(
        "sample_id",
        type=str,
