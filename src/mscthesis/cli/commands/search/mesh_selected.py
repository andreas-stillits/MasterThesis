...
def execute_meshing(sample_id: str) -> None:
    ...
    # check if all mesh variants exist already before building model
    stomatal_aspect_set = config.search.stomatal_aspect_set.copy()
    all_exists = True

    for specifier in stomatal_aspect_set:
        mesh_path = sample_paths.meshing(specifier).mesh
        if not mesh_path.exists():
            all_exists = False
            break

    if not all_exists or force:

        plug_aspect: float | None = None

        for specifier, stomatal_aspect in stomatal_aspect_set.items():
        ...
        if plug_aspect is not None:
            # run an additional iteration where stomatal area ~plug area using the 0 specifier
            specifier: int = config.search.selected.porous_inlet_specifier
            stomatal_aspect: float = (
                plug_aspect - config.search.selected.porous_inlet_offset
            )
            meshing_paths = sample_paths.meshing(specifier)
            meshing_paths.root.ensure()
            if not meshing_paths.mesh.exists() or force:

                airspace_tag, _ = build_sample_model(
                    sample_paths.triangulation.cadmodel.require(),
                    config.meshing.boundary_margin,
                    config.meshing.substomatal_margin,
                    config.meshing.atol,
