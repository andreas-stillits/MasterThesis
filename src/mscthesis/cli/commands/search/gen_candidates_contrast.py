...
from ....core.synthesis.contrast import generate_voxels_from_rng
    ...
    for num_cells in config.search.candidates.num_cells_set:
        for radius_center in config.search.candidates.radius_center_set:
            for plug_aspect in config.search.plug_aspect_set.values():
                for division in config.search.candidates.division_set:
...
def create_candidate(taskstream: TaskStream) -> None:
    ...
    candidate_config["synthesis"]["contrast"]["num_cells"] = candidate.num_cells
    candidate_config["synthesis"]["contrast"]["radius_min"] = candidate.radius_min
    candidate_config["synthesis"]["contrast"]["radius_max"] = candidate.radius_max
    candidate_config["synthesis"]["contrast"]["division"] = candidate.division
    ...
    dump_manifest(
        ...
        command_name="gen-candidates-contrast",
...
def add_parser(subparsers: argparse._SubParsersAction) -> None:
    parser = subparsers.add_parser(
        "gen-candidates-contrast", help="Generate candidate configurations for search"
    )
